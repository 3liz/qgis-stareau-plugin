...
class PipesWaterIntakeToTreatment(BaseDatabaseAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        query_captages = sql.SQL("""
            SELECT fid FROM {schemaname}.aep_captage
        """).format(schemaname=sql.Identifier(schema_aep))

        # psycopg2 connection
        conn = connect(uri.connectionInfo())
        # psycopg2 rcomposed request as string
        captage_fids = connection.executeSql(query_captages.as_string(conn))
        ...
        for fid in captage_fids:
            query_canal = sql.SQL("""
                ...
                {f}, {saep}::text, 'aep_traitement'::text)
            """).format(sg=sql.Identifier(schema_global), f=sql.Literal(fid[0]), saep=sql.Literal(schema_aep))
