...
def get_postgres_layers(id_layer: str, action_name: str) -> None | QgsVectorLayer:
    ...
    if layer is None:
        display_error_message(
            f"Erreur dans l'action \"{action_name}\", la couche {id_layer} n'existe pas dans le projet !"
        )
        return None
    if not layer.isValid():
        display_error_message(f'Erreur dans l\'action "{action_name}", la couche {id_layer} est invalide !')
        return None
    if layer.dataProvider().name() != "postgres":
        display_error_message(
            f"Erreur dans l'action \"{action_name}\", la couche {id_layer} n'est pas une couche Postgres !"
        )
        return None
